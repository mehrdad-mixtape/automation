...
def set_eigrp(token):
    """
	Enable EIGRP on CSR 1000V router.
    """
    url = 'https://%s:55443/api/v1/routing-svc/eigrp' % (ip_address)
    headers={ 'Content-Type': 'application/json', 'Accept':'application/json', 'X-auth-token': token}

    payload = {
	"routing-protocol-id": "100"
        }
    url2 = 'https://%s:55443//api/v1/routing-svc/eigrp/100/networks' % (ip_address)
    payload2 = {
    ...
    print('The router responds2 with status code: %s ' % response2.status_code)
