        ...
        while True:
            ...
            for notified_socket in read_sockets: # search in read_sockets.
                ...
                ################################ for second time we check client that send message.################################
                    ...
                    if message is False:
                        print(f"Closed connection from '{self.clients[notified_socket]['data'][0]}'") # clients={client_socket:{"header": message_header, "data": client_socket.recv(message_length)}, ...}
                        self.sockets_list.remove(notified_socket)
                        del self.clients[notified_socket]
                        continue

                    user = self.clients[notified_socket] # I access to {"header": message_header, "data": client_socket.recv(message_length)}
                    print(f"{self.Server_time()} message from '{user['data'][0]}':  {message['data']}")
